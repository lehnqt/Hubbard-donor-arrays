function psi_j = cj2d(N_size, N_up, N_down, w, j,k,sigma)
N_row=N_size(1);
N_col=N_size(2);
N_sites=N_row*N_col;
...
if sigma==1
    ...
    [~,row_index_up,column_index_up]=intersect(dec_up - 2^(N_sites-j)*basis_up(:,j),dec_up_2);
...
elseif sigma== -1
    ...
    [~,row_index_down,column_index_down]=intersect(dec_down - 2^(N_sites-j)*basis_down(:,j),dec_down_2);
